...
+currently assume ``browsertab.AbstractTab``.
+"""
+
...
+
+
+def _workspace_fields(widget: tabwidget.TabWidget, idx: int, tab: WorkspaceTab):
...
+
+def install() -> None:
+    """Install the native workspace branches into qutebrowser tab management."""
+    global _installed
+    if _installed:
+        return
+    _installed = True
...
+            and modeman.instance(self._win_id).mode not in modeman.PROMPT_MODES
+        ):
+            modeman.enter(self._win_id, tab.data.input_mode, "restore workspace tab")
+
...
+
+    def tb_remove_tab(self, tab, *, add_undo=True, new_undo=True, crashed=False):
+        if not isinstance(tab, WorkspaceTab):
...
+                self, url=url, background=False, related=related, idx=idx
+            )
+            self.widget.setCurrentWidget(current)
+            current.setFocus()
+            return tab
+        return original_tb_tabopen(
+            self, url=url, background=background, related=related, idx=idx
+        )
+
...
+
+    def tabopen_workspace(self, content, background=False, related=False, idx=None):
...
+                content, background=background, related=related, idx=idx
+            )
+            window.show()
+            return tab
+
